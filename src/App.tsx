...
function NewBugModal({ onClose, onCreated }: { onClose: () => void; onCreated: (id: string) => void }) {
  ...
  const runAITriage = () => {
    if (!aiLog.trim()) return; setIsAnalyzing(true);
    setTimeout(() => {
      setIsAnalyzing(false); const lowerLog = aiLog.toLowerCase();
      if (lowerLog.includes('null') || lowerLog.includes('undefined') || lowerLog.includes('crash')) { setTitle('Malenia\'s Rot Exception: Fatal System Collapse'); setSeverity('Critical'); setPriority('P0'); setEnvironment('Limgrave (Prod)'); setTagsStr('curse, fatal'); setStoryPoints(13); } else if (lowerLog.includes('network') || lowerLog.includes('cors') || lowerLog.includes('500')) { setTitle('Erdtree Connection Severed (CORS/500)'); setSeverity('Major'); setPriority('P1'); setTagsStr('magic, network'); setStoryPoints(5); } else { setTitle('Unidentified Anomaly in the Realm'); setTagsStr('triage-needed'); }
      setDescription(`**Guidance of Grace Report:**\n\n\`\`\`bash\n${aiLog.slice(0, 300)}${aiLog.length > 300 ? '...' : ''}\n\`\`\`\n\n**Suggested Rite:** Review the incantations above. Cleanse the rot.`);
      setMode('manual');
    }, 2000);
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4 backdrop-blur-sm">
      <div className="flex w-full max-w-4xl flex-col overflow-hidden border border-[hsl(var(--primary)/.4)] bg-[hsl(var(--card)/.95)] backdrop-blur-xl shadow-[0_0_50px_rgba(197,168,101,0.15)]">
        <div className="flex items-center justify-between border-b border-[hsl(var(--border)/.5)] bg-[hsl(var(--background)/.5)] px-6 py-4">
          <div className="flex items-center gap-3"><span className="flex h-8 w-8 items-center justify-center border border-[hsl(var(--primary)/.3)] bg-[hsl(var(--primary)/.1)] text-[hsl(var(--primary))]"><Skull size={18} /></span><div><p className="font-serif text-[10px] font-bold uppercase tracking-widest text-[hsl(var(--muted-foreground))]">Realm of {currentWs?.key}</p><h2 className="text-base font-serif font-bold text-[hsl(var(--primary))] erdtree-text">Declare Great Foe <span className="text-[10px] text-[hsl(var(--muted-foreground))] tracking-normal">(New Issue)</span></h2></div></div>
          <div className="flex items-center gap-2 border border-[hsl(var(--border)/.5)] bg-[hsl(var(--muted)/.5)] p-1">
            <button onClick={() => setMode('manual')} className={`flex items-center gap-2 px-3 py-1.5 text-xs font-serif font-bold transition-all ${mode === 'manual' ? 'bg-[hsl(var(--card))] text-[hsl(var(--primary))] border border-[hsl(var(--primary)/.3)]' : 'text-[hsl(var(--muted-foreground))] hover:text-[hsl(var(--foreground))] border border-transparent'}`}><Terminal size={14}/> Scribe Manual</button>
            <button onClick={() => setMode('ai')} className={`flex items-center gap-2 px-3 py-1.5 text-xs font-serif font-bold transition-all ${mode === 'ai' ? 'bg-[hsl(var(--primary)/.15)] text-[hsl(var(--primary))] border border-[hsl(var(--primary)/.5)]' : 'text-[hsl(var(--muted-foreground))] hover:text-[hsl(var(--foreground))] border border-transparent'}`}><Flame size={14}/> Guidance of Grace</button>
          </div>
        ...
        <div className="flex min-h-[500px] flex-col md:flex-row">
          {mode === 'ai' ? (
            <div className="flex flex-1 flex-col items-center justify-center p-8 text-center relative overflow-hidden">
              <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[400px] h-[400px] bg-[hsl(var(--primary)/.05)] rounded-full blur-[80px] pointer-events-none"></div>
              <div className="mb-6 flex h-20 w-20 items-center justify-center rounded-full border border-[hsl(var(--primary)/.3)] bg-[hsl(var(--primary)/.1)] text-[hsl(var(--primary))] erdtree-glow z-10"><Flame size={32} /></div>
              <h3 className="mb-2 text-2xl font-serif font-bold text-[hsl(var(--primary))] erdtree-text z-10">Consult the Two Fingers</h3>
              <p className="mb-6 max-w-md text-sm font-serif text-[hsl(var(--muted-foreground))] z-10">Offer a raw curse log or scattered reports. The Erdtree's grace will decipher its meaning and draft the writ.</p>
              <textarea value={aiLog} onChange={e => setAiLog(e.target.value)} disabled={isAnalyzing} placeholder="Paste the cursed texts here..." className="focus-ring mb-6 h-40 w-full max-w-2xl resize-none border border-[hsl(var(--primary)/.3)] bg-[hsl(var(--background)/.5)] p-4 font-mono text-xs outline-none text-[hsl(var(--primary))] z-10" />
              <button onClick={runAITriage} disabled={isAnalyzing || !aiLog.trim()} className="flex items-center gap-2 border border-[hsl(var(--primary))] bg-[hsl(var(--primary)/.15)] px-8 py-3 text-sm font-serif font-bold text-[hsl(var(--primary))] transition-all hover:bg-[hsl(var(--primary))] hover:text-black disabled:opacity-50 z-10">
                {isAnalyzing ? <RefreshCw className="animate-spin" size={16}/> : <Sparkles size={16}/>}
                {isAnalyzing ? 'Communing... (Analyzing)' : 'Seek Guidance (Auto-Draft)'}
              </button>
            </div>
          ) : (
            <>
              <div className="flex flex-1 flex-col overflow-y-auto border-r border-[hsl(var(--border)/.5)] p-5">
                ...
                <div className="mb-4 flex flex-wrap items-center gap-2 border-b border-[hsl(var(--border)/.5)] pb-3"><span className="text-[10px] font-bold uppercase tracking-widest text-[hsl(var(--muted-foreground))] mr-2 font-serif">Sacred Texts:</span><button onClick={() => setDescription(BUG_TEMPLATE)} className="flex items-center gap-1.5 border border-[hsl(var(--border))] bg-[hsl(var(--muted)/.5)] px-2 py-1 text-[10px] font-bold font-serif text-[hsl(var(--foreground))] hover:border-[hsl(var(--primary))] transition-colors"><Skull size={12}/> Anomaly Report</button><button onClick={() => setDescription(FEATURE_TEMPLATE)} className="flex items-center gap-1.5 border border-[hsl(var(--border))] bg-[hsl(var(--muted)/.5)] px-2 py-1 text-[10px] font-bold font-serif text-[hsl(var(--foreground))] hover:border-[hsl(var(--primary))] transition-colors"><FileText size={12}/> Lord's Decree</button></div>
                <textarea value={description} onChange={(e) => setDescription(e.target.value)} placeholder="Inscribe the lore of this encounter, the rites to reproduce it, and the curse it brings upon the realm..." className="mb-4 min-h-[120px] w-full flex-1 resize-none bg-transparent font-serif text-sm leading-relaxed outline-none placeholder:text-[hsl(var(--muted-foreground)/.4)] text-[hsl(var(--foreground)/.9)]" />
                <div className="mt-auto space-y-4 border-t border-[hsl(var(--border)/.5)] pt-4">
                  <div><label className="mb-1.5 flex items-center gap-1.5 text-[10px] font-bold uppercase tracking-widest font-serif text-[hsl(var(--muted-foreground))]"><Code2 size={12}/> Optional: Incantation (Git Patch)</label><textarea value={codePatch} onChange={e => setCodePatch(e.target.value)} placeholder="@@ -1,3 +1,4 @@... inscribe the spell here" className="h-16 w-full resize-none border border-[hsl(var(--border)/.5)] bg-[hsl(var(--background)/.5)] p-2 font-mono text-[10px] outline-none placeholder:text-[hsl(var(--muted-foreground)/.3)] text-[hsl(var(--primary))]" /></div>
                  <div><p className="mb-2 text-[10px] font-bold uppercase tracking-widest font-serif text-[hsl(var(--muted-foreground))]">Visions <span className="tracking-normal font-sans normal-case opacity-70">(Attachments)</span></p><div className="flex flex-wrap gap-2 mb-2">
              ...
              <div className="w-full shrink-0 overflow-y-auto bg-[hsl(var(--background)/.3)] p-5 md:w-[300px] border-l border-[hsl(var(--border)/.5)]">
                <h3 className="mb-4 text-xs font-bold uppercase tracking-widest font-serif text-[hsl(var(--primary))]">Attributes</h3>
                <div className="space-y-4">
                  <div><label className="mb-1 block text-[10px] font-bold uppercase font-serif text-[hsl(var(--muted-foreground))]">Summoned <span className="tracking-normal font-sans normal-case opacity-70">(Assignee)</span></label><select value={assigneeId} onChange={e => setAssigneeId(e.target.value)} className="w-full border border-[hsl(var(--border)/.5)] bg-[hsl(var(--background)/.5)] px-2 py-1.5 text-xs font-serif font-bold shadow-sm outline-none text-[hsl(var(--foreground))]"><option value="">No summon sign</option>{(store.users || []).map((u) => <option key={u.id} value={u.id}>{u.name}</option>)}</select></div>
                  <div className="grid grid-cols-2 gap-2"><div><label className="mb-1 block text-[10px] font-bold uppercase font-serif text-[hsl(var(--muted-foreground))]">Threat <span className="block tracking-normal font-sans normal-case opacity-70">(Severity)</span></label><select value={severity} onChange={e => setSeverity(e.target.value as Severity)} className="w-full border border-[hsl(var(--border)/.5)] bg-[hsl(var(--background)/.5)] px-2 py-1.5 text-[11px] font-serif font-bold shadow-sm outline-none">{Object.keys(severityMeta).map((v) => <option key={v}>{severityMeta[v as Severity].label}</option>)}</select></div><div><label className="mb-1 block text-[10px] font-bold uppercase font-serif text-[hsl(var(--muted-foreground))]">Urgency <span className="block tracking-normal font-sans normal-case opacity-70">(Priority)</span></label><select value={priority} onChange={e => setPriority(e.target.value as Priority)} className="w-full border border-[hsl(var(--border)/.5)] bg-[hsl(var(--background)/.5)] px-2 py-1.5 text-[11px] font-serif font-bold shadow-sm outline-none">{['P0', 'P1', 'P2', 'P3'].map((v) => <option key={v}>{v === 'P0' ? 'Urgent' : v}</option>)}</select></div></div>
                  <div><label className="mb-1 block text-[10px] font-bold uppercase font-serif text-[hsl(var(--muted-foreground))]">Region <span className="tracking-normal font-sans normal-case opacity-70">(Component)</span></label><select value={componentId} onChange={e => setComponentId(e.target.value)} className="w-full border border-[hsl(var(--border)/.5)] bg-[hsl(var(--background)/.5)] px-2 py-1.5 text-xs font-serif font-bold shadow-sm outline-none">{wsComponents.map((c) => <option key={c.id} value={c.id}>{c.name}</option>)}</select></div>
                  
                  <div className="my-4 h-px bg-[hsl(var(--border)/.5)]" />
                  <h3 className="mb-4 text-xs font-bold uppercase tracking-widest font-serif text-[hsl(var(--primary))]">Lore & Conditions</h3>
                  
                  <div className="grid grid-cols-2 gap-2"><div><label className="mb-1 flex items-center gap-1.5 text-[10px] font-bold uppercase font-serif text-[hsl(var(--muted-foreground))]"><Hash size={10}/> Yield <span className="tracking-normal font-sans normal-case opacity-70">(Points)</span></label><select value={storyPoints} onChange={e => setStoryPoints(e.target.value)} className="w-full border border-[hsl(var(--border)/.5)] bg-[hsl(var(--background)/.5)] px-2 py-1.5 text-[11px] font-serif font-bold shadow-sm outline-none"><option value="">None</option>{storyPointOptions.map((v) => <option key={v} value={v}>{v}</option>)}</select></div><div><label className="mb-1 flex items-center gap-1.5 text-[10px] font-bold uppercase font-serif text-[hsl(var(--muted-foreground))]"><Calendar size={10}/> Day <span className="tracking-normal font-sans normal-case opacity-70">(Due)</span></label><input type="date" value={dueDate} onChange={e => setDueDate(e.target.value)} className="w-full border border-[hsl(var(--border)/.5)] bg-[hsl(var(--background)/.5)] px-2 py-1.5 text-[11px] font-serif font-bold shadow-sm outline-none" /></div></div>
                  <div><label className="mb-1 flex items-center gap-1.5 text-[10px] font-bold uppercase font-serif text-[hsl(var(--muted-foreground))]"><Monitor size={10}/> Realm <span className="tracking-normal font-sans normal-case opacity-70">(Env)</span></label><select value={environment} onChange={e => setEnvironment(e.target.value)} className="w-full border border-[hsl(var(--border)/.5)] bg-[hsl(var(--background)/.5)] px-2 py-1.5 text-xs font-serif font-bold shadow-sm outline-none">{envOptions.map((v) => <option key={v}>{v}</option>)}</select></div>
                  <div><label className="mb-1 block text-[10px] font-bold uppercase font-serif text-[hsl(var(--muted-foreground))]">Manifestation <span className="tracking-normal font-sans normal-case opacity-70">(Repro)</span></label><select value={reproducibility} onChange={e => setReproducibility(e.target.value as Reproducibility)} className="w-full border border-[hsl(var(--border)/.5)] bg-[hsl(var(--background)/.5)] px-2 py-1.5 text-[11px] font-serif font-bold shadow-sm outline-none">{reproOptions.map((v) => <option key={v}>{v}</option>)}</select></div>
                  <div><label className="mb-1 flex items-center gap-1.5 text-[10px] font-bold uppercase font-serif text-[hsl(var(--muted-foreground))]"><Tag size={10}/> Sigils <span className="tracking-normal font-sans normal-case opacity-70">(Tags)</span></label><input value={tagsStr} onChange={e => setTagsStr(e.target.value)} placeholder="e.g. magic, combat" className="w-full border border-[hsl(var(--border)/.5)] bg-[hsl(var(--background)/.5)] px-2 py-1.5 text-[11px] font-serif font-bold shadow-sm outline-none text-[hsl(var(--primary))]" /></div>
                </div>
              </div>
            </>
          )}
        ...
        <div className="flex items-center justify-between border-t border-[hsl(var(--border)/.5)] bg-[hsl(var(--background)/.5)] px-6 py-4">
          <p className="text-[10px] font-serif text-[hsl(var(--muted-foreground))]">The scrolls accept ancient Markdown formatting.</p>
          <div className="flex gap-2">
            <button onClick={onClose} className="border border-[hsl(var(--border))] bg-[hsl(var(--background)/.5)] px-5 py-2 text-xs font-bold font-serif hover:bg-[hsl(var(--muted)/.5)] hover:text-[hsl(var(--primary))] transition-colors">Flee</button>
            <button onClick={submitAction} disabled={!title.trim() || mode === 'ai'} className="bg-[hsl(var(--primary))] px-6 py-2 text-xs font-bold font-serif text-black shadow-[0_0_15px_rgba(197,168,101,0.4)] hover:brightness-125 disabled:opacity-50 transition-all">Declare Foe</button>
          </div>
        </div>
      </div>
    </div>
          ...
          <section className="mb-7">
            {bug.codePatch ? (
              ...
              <div className="flex flex-col items-center justify-center p-16 border border-dashed border-[hsl(var(--primary)/.4)] bg-[hsl(var(--primary)/.05)] text-center backdrop-blur-md">
                <div className="mb-6 flex h-20 w-20 items-center justify-center rounded-full border border-[hsl(var(--primary)/.3)] bg-[hsl(var(--primary)/.1)] text-[hsl(var(--primary))] erdtree-glow"><Flame size={32} /></div>
                <h3 className="font-serif font-bold text-2xl mb-3 text-[hsl(var(--primary))] erdtree-text">No Incantation Present</h3>
                <p className="text-sm font-serif text-[hsl(var(--muted-foreground))] mb-10 max-w-md">Let the Guidance of Grace analyze this foe's lore and automatically stream a restorative Git Diff incantation to mend the realm.</p>
                <button onClick={generateAIFix} disabled={isGeneratingFix} className="flex items-center gap-3 border border-[hsl(var(--primary))] bg-[hsl(var(--primary)/.15)] px-8 py-3.5 text-sm font-serif font-bold uppercase tracking-widest text-[hsl(var(--primary))] shadow-[0_0_20px_rgba(197,168,101,0.2)] transition-all hover:bg-[hsl(var(--primary))] hover:text-black disabled:opacity-50">
                  {isGeneratingFix ? <RefreshCw className="animate-spin" size={16}/> : <Sparkles size={16}/>}
                  {isGeneratingFix ? 'Communing with Erdtree...' : 'Seek Guidance (Auto-Fix)'}
                </button>
              </div>
            )}
          </section>
