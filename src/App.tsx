  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-start justify-center bg-black/60 pt-[15vh] backdrop-blur-sm" onClick={() => setOpen(false)}>
      <div className="w-full max-w-xl overflow-hidden border border-[hsl(var(--primary)/.3)] bg-[hsl(var(--card)/.9)] backdrop-blur-xl shadow-[0_0_50px_rgba(197,168,101,0.15)]" onClick={e => e.stopPropagation()}>
        <div className="flex items-center border-b border-[hsl(var(--border)/.5)] px-4 py-3"><SearchIcon size={18} className="text-[hsl(var(--primary))]" /><input autoFocus value={search} onChange={e => setSearch(e.target.value)} placeholder="Search the annals of history..." className="flex-1 bg-transparent px-3 py-1 text-xl font-serif outline-none placeholder:text-[hsl(var(--muted-foreground))]" /><kbd className="bg-[hsl(var(--muted))] px-1.5 font-mono text-[10px] text-[hsl(var(--muted-foreground))]">ESC</kbd></div>
        <div className="max-h-[300px] overflow-y-auto p-2">
          {results.length === 0 ? <p className="p-4 text-center text-lg font-serif text-[hsl(var(--muted-foreground))]">No records found in the Lands Between.</p> : (
            results.map(bug => <button key={bug.id} onClick={() => { setLocation(`/bugs/${bug.id}`); setOpen(false); }} className="flex w-full items-center gap-3 px-3 py-2.5 text-left hover:bg-[hsl(var(--muted)/.5)]"><span className="font-mono text-xs font-bold text-[hsl(var(--primary))]">{bug.key}</span><span className="truncate text-xl font-semibold font-serif">{bug.title}</span><StageBadge stage={bug.stage} compact /></button>)
          )}
        </div>
      </div>
    </div>
  ...
  return (
    <div className="fixed inset-0 z-[200] flex flex-col items-center justify-center bg-[#000000] text-white p-6 animate-in fade-in duration-1000 relative overflow-hidden">
      ...
      <div className="text-center w-full max-w-2xl z-10">
        <h3 className="font-serif text-[27px] tracking-[0.3em]  text-[hsl(var(--destructive))] mb-12 drop-shadow-[0_0_15px_rgba(255,0,0,0.5)]">Enemy Felled... Soon</h3>
        <h1 className="text-8xl font-serif font-bold  mb-8 text-[hsl(var(--primary))] drop-shadow-[0_0_30px_rgba(197,168,101,0.3)]">{minutes}:{seconds}</h1>
        <button onClick={toggleTimer} className={`mb-16 flex mx-auto items-center gap-3 border border-[hsl(var(--primary))] px-8 py-3.5 text-xl font-serif font-bold  tracking-wider transition-all ${isActive ? 'bg-black text-[hsl(var(--primary))]' : 'bg-[hsl(var(--primary))] text-black shadow-[0_0_30px_rgba(197,168,101,0.5)] hover:bg-[hsl(var(--primary)/.9)]'}`}>
    ...
    <aside className={`fixed inset-y-0 left-0 z-40 flex w-[260px] flex-col border-r border-[hsl(var(--border)/.5)] bg-[hsl(var(--sidebar)/.8)] backdrop-blur-xl px-5 py-6 text-[hsl(var(--sidebar-foreground))] transition-transform duration-200 md:translate-x-0 ${mobileOpen ? 'translate-x-0' : '-translate-x-full'}`}>
      <div className="flex items-center justify-between px-1">
        <Link href="/" className="flex items-center gap-3 text-[hsl(var(--sidebar-foreground))] no-underline">
          ...
          <span><span className="block text-xl font-serif font-bold tracking-wider erdtree-text">Erdtree Order</span></span>
        </Link>
        <button onClick={() => setMobileOpen(false)} className="md:hidden text-[hsl(var(--muted-foreground))]"><X size={20} /></button>
      </div>

      <div className="relative mt-10">
        <p className="mb-2 px-1 font-serif text-[14px]  text-[hsl(var(--muted-foreground))]">Current Realm <span className="font-sans tracking-normal opacity-70">(Workspace)</span></p>
        <button onClick={() => setWorkspaceMenuOpen(!workspaceMenuOpen)} className="flex w-full items-center gap-3 border border-[hsl(var(--sidebar-border)/.5)] bg-[hsl(var(--sidebar-accent)/.5)] p-3 text-left hover:border-[hsl(var(--primary)/.5)] transition-all">
          ...
          <div className="min-w-0 flex-1"><p className="truncate text-lg font-serif font-bold text-[hsl(var(--foreground))]">{currentWorkspace?.name ?? 'Select Realm'}</p></div>
        ...
        {workspaceMenuOpen && (
          <div className="absolute left-0 top-16 z-50 w-full border border-[hsl(var(--primary)/.3)] bg-[hsl(var(--sidebar))] p-2 shadow-2xl text-[hsl(var(--foreground))]">
            <div className="space-y-1">{userWorkspaces.map((ws) => <button key={ws.id} onClick={() => { switchWorkspace(ws.id); setWorkspaceMenuOpen(false); }} className={`flex w-full items-center justify-between px-3 py-2 text-lg font-serif font-bold transition-colors ${ws.id === currentWorkspace?.id ? 'bg-[hsl(var(--primary)/.15)] text-[hsl(var(--primary))]' : 'hover:bg-[hsl(var(--muted))]'}`}><span className="truncate">{ws.name}</span></button>)}</div>
            <button onClick={() => { setCreateWsOpen(true); setWorkspaceMenuOpen(false); }} className="mt-2 flex w-full items-center gap-2 border-t border-[hsl(var(--border))] px-3 pt-3 text-lg font-serif font-bold text-[hsl(var(--primary))] hover:brightness-125"><Plus size={14} /> Establish New Realm</button>
          </div>
        )}
      </div>

      <nav className="mt-8 space-y-1" aria-label="Main navigation">
        <p className="mb-3 px-1 font-serif text-[14px]  text-[hsl(var(--muted-foreground))]">Journey</p>
        {nav.map(({ href, label, en, icon: Icon }) => (
          <Link key={href} href={href} onClick={() => setMobileOpen(false)} className={`flex items-center gap-3 px-3 py-3 text-[19px] font-serif font-bold no-underline transition-all ${location === href ? 'bg-[hsl(var(--primary)/.1)] border-l-2 border-[hsl(var(--primary))] text-[hsl(var(--primary))]' : 'text-[hsl(var(--muted-foreground))] hover:bg-[hsl(var(--muted)/.5)] hover:text-[hsl(var(--foreground))] border-l-2 border-transparent'}`}>
      ...
      <div className="mt-auto border-t border-[hsl(var(--border)/.5)] pt-6">
        <div className="flex items-center gap-3 px-1">
          ...
          <div className="min-w-0"><p className="truncate text-lg font-serif font-bold">{currentUser?.name}</p><p className="truncate font-mono text-[9px] text-[hsl(var(--muted-foreground))]">@{currentUser?.username}</p></div>
        ...
        <button onClick={resetDemo} className="mt-6 flex w-full items-center gap-3 px-1 text-left font-serif text-[15px] font-bold  text-[hsl(var(--muted-foreground))] hover:text-[hsl(var(--destructive))] transition-colors ">
        ...
        <div className="relative ml-auto flex items-center gap-4">
          ...
          {profileOpen && (
            <div className="absolute right-0 top-12 z-30 w-64 border border-[hsl(var(--primary)/.3)] bg-[hsl(var(--sidebar))] p-4 shadow-2xl">
              <div className="flex items-center gap-3 mb-4 pb-4 border-b border-[hsl(var(--border))]"><Avatar user={currentUser} size="md" /><div className="min-w-0"><p className="truncate text-xl font-serif font-bold">{currentUser?.name}</p><p className="truncate font-mono text-[10px] text-[hsl(var(--muted-foreground))]">@{currentUser?.username}</p></div></div>
              <button onClick={() => void logout()} className="flex w-full items-center gap-2 px-2 py-2 text-lg font-serif font-bold  text-[hsl(var(--destructive))] hover:bg-[hsl(var(--destructive)/.1)] transition-colors"><LogOut size={14} /> Sever Connection <span className="font-sans normal-case tracking-normal opacity-70">(Logout)</span></button>
            </div>
          )}
        </div>
      </header>
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4 backdrop-blur-sm">
      <form onSubmit={handleSubmit} className="w-full max-w-md border border-[hsl(var(--primary)/.4)] bg-[hsl(var(--card)/.9)] backdrop-blur-xl p-8 shadow-[0_0_40px_rgba(197,168,101,0.15)]">
        <div className="flex items-center justify-between mb-6"><h2 className="text-[27px] font-serif font-bold erdtree-text">Discover New Realm <span className="text-xs font-sans tracking-normal opacity-70 text-[hsl(var(--muted-foreground))]">(New Workspace)</span></h2><button type="button" onClick={onClose} className="text-[hsl(var(--muted-foreground))] hover:text-[hsl(var(--primary))]"><X size={20} /></button></div>
        <div className="space-y-5">
          <label className="block text-lg font-serif font-bold  text-[hsl(var(--muted-foreground))] mb-2">Realm Name<input value={name} onChange={(e) => { setName(e.target.value); if (!key) setKey(e.target.value.slice(0, 3).toUpperCase()); }} placeholder="e.g. Limgrave" className="mt-2 block w-full border border-[hsl(var(--border))] bg-[hsl(var(--background)/.5)] px-3 py-3 text-sm font-sans outline-none focus:border-[hsl(var(--primary))] text-[hsl(var(--foreground))]" /></label>
          <label className="block text-lg font-serif font-bold  text-[hsl(var(--muted-foreground))] mb-2">Sigil Prefix<input value={key} onChange={(e) => setKey(e.target.value.toUpperCase())} maxLength={5} placeholder="e.g. LMG" className="mt-2 block w-full border border-[hsl(var(--border))] bg-[hsl(var(--background)/.5)] px-3 py-3 text-sm font-mono outline-none focus:border-[hsl(var(--primary))] text-[hsl(var(--foreground))]" /></label>
        </div>
        <div className="mt-8 flex justify-end gap-3">
          <button type="button" onClick={onClose} className="px-5 py-2.5 text-lg font-serif font-bold  hover:text-[hsl(var(--primary))] transition-colors">Flee <span className="font-sans normal-case tracking-normal opacity-70">(Cancel)</span></button>
          <button type="submit" disabled={!name || !key} className="bg-[hsl(var(--primary))] px-6 py-2.5 text-lg font-serif font-bold  text-black shadow-[0_0_15px_rgba(197,168,101,0.3)] hover:brightness-125 disabled:opacity-50 transition-all">Establish</button>
        </div>
      </form>
    </div>
  );
...
function sanitizeUsername(value: string) {
  ...
  return cleaned || 'tarnished';
}

function uniqueLocalUsername(
  requested: string,
  users: User[],
  userId: string,
) {
  const base = sanitizeUsername(requested);

  const collision = users.some(
    (user) =>
      user.id !== userId &&
      user.username.toLowerCase() === base.toLowerCase(),
  );
...
function profileFromSupabaseUser(
  ...
  const emailPrefix =
    authUser.email?.split('@')[0] ?? `tarnished-${authUser.id.slice(0, 6)}`;
...
function ensureAuthenticatedStore(
  current: Store,
  authUser: SupabaseUser,
): Store {
  const currentUsers = current.users ?? [];

  const generatedProfile = profileFromSupabaseUser(
    authUser,
    currentUsers,
  );
  ...
  const localUser: User = existing
    ? {
        ...existing,
        // Preserve a username the user already had in this browser,
        // but refresh auth-derived identity details.
        name: existing.name || generatedProfile.name,
        username:
          existing.username || generatedProfile.username,
        email: generatedProfile.email,
        avatarUrl:
          generatedProfile.avatarUrl ?? existing.avatarUrl,
        provider: generatedProfile.provider,
      }
    : generatedProfile;

  const users = existing
    ? currentUsers.map((user) =>
        user.id === localUser.id ? localUser : user,
      )
  ...
  const existingUserWorkspaces = workspaces.filter((workspace) =>
    workspace.memberIds.includes(localUser.id),
  );

  if (existingUserWorkspaces.length > 0) {
    const currentWorkspaceStillValid =
      current.currentWorkspaceId &&
      existingUserWorkspaces.some(
        (workspace) =>
          workspace.id === current.currentWorkspaceId,
      );
    ...
    return {
      ...
      currentWorkspaceId: currentWorkspaceStillValid
        ? current.currentWorkspaceId
        : existingUserWorkspaces[0].id,
    };
  }

  const baseName =
    localUser.name.trim() || localUser.username || 'Tarnished';

  const newWorkspaceId = uid('ws');
  const newProductId = uid('prod');

  const baseKey =
    sanitizeUsername(baseName)
      .replace(/[^a-z0-9]/g, '')
      .slice(0, 3)
      .toUpperCase() || 'RLM';
  ...
  const newWorkspace: Workspace = {
    id: newWorkspaceId,
    name: `Realm of ${baseName}`,
    key: baseKey,
    ownerId: localUser.id,
    memberIds: [localUser.id],
  };
...
function AppProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    const applySessionUser = (user: SupabaseUser | null) => {
      if (!alive) return;

      setAuthUser(user);

      if (user) {
        setStore((current) => ensureAuthenticatedStore(current, user));
      } else {
        setStore((current) => ({
          ...current,
          currentUserId: null,
  ...
  const resetDemo = () => {
    ...
    if (authUser) {
      setStore(ensureAuthenticatedStore(seedStore(), authUser));
    } else {
      setStore(seedStore());
    }
  };
  ...
  const createWorkspace = (name: string, key: string) => {
    if (!store.currentUserId) {
      throw new Error(
        'Cannot create a workspace without an authenticated user.',
      );
    }
    ...
    setStore((current) => ({
      ...
      products: [...(current.products || []), newProd],
  ...
  if (authLoading) {
    return (
      <WorkspaceContext.Provider value={contextValue}>
        <GlobalStyles />
        <div className="flex min-h-screen items-center justify-center bg-[#0a0a0a] text-[hsl(var(--primary))]">
...
function SectionHeader({ eyebrow, title, description, action }: { eyebrow?: string; title: string; description?: string; action?: ReactNode }) {
  return <div className="mb-8 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div><p className="font-serif text-[15px] font-bold  text-[hsl(var(--primary))] mb-2">{eyebrow}</p><h1 className="text-3xl font-serif font-bold uppercase tracking-widest text-[hsl(var(--foreground))] erdtree-text">{title}</h1>{description && <p className="mt-3 max-w-2xl text-[19px] font-serif leading-relaxed text-[hsl(var(--muted-foreground))]">{description}</p>}</div>{action}</div>;
}
...
function BugDetailPage() {
  ...
  return <div className="mx-auto w-full max-w-[1600px] px-5 py-8 md:px-10 lg:px-12 relative z-10">
    ...
    <div className="mb-6 flex items-center justify-between"><button onClick={() => setLocation('/')} className="flex items-center gap-2 text-lg font-serif font-bold  text-[hsl(var(--muted-foreground))] hover:text-[hsl(var(--primary))] transition-colors"><ArrowLeft size={15} /> Flee Battle</button><div className="flex items-center gap-3">
      <button onClick={() => setZenMode(true)} className="flex items-center gap-2 border border-[hsl(var(--primary))] bg-[hsl(var(--primary)/.1)] px-5 py-2.5 text-lg font-serif font-bold  text-[hsl(var(--primary))] shadow-[0_0_15px_rgba(197,168,101,0.2)] transition-all hover:bg-[hsl(var(--primary))] hover:text-black"><Swords size={14}/> Traverse Fog <span className="font-sans normal-case tracking-normal opacity-70 text-[9px]">(Focus)</span></button>
      <button onClick={() => setEditing(!editing)} className="flex items-center gap-2 border border-[hsl(var(--border)/.5)] bg-[hsl(var(--card)/.6)] backdrop-blur-md px-5 py-2.5 text-lg font-serif font-bold  hover:bg-[hsl(var(--muted)/.8)] transition-colors"><Settings2 size={14} /> {editing ? 'Seal Edits' : 'Edit Foe'}</button>
    </div></div>
    
    <div className="grid gap-8 lg:grid-cols-[minmax(0,1fr)_340px]">
      <div className="min-w-0">
        <div className="mb-8"><div className="flex items-start gap-4"><div className="mt-1 flex h-12 w-12 shrink-0 items-center justify-center border border-[hsl(var(--primary)/.4)] bg-[hsl(var(--primary)/.1)] text-[hsl(var(--primary))] shadow-inner"><Skull size={24} /></div><div className="min-w-0 flex-1"><h1 className="text-4xl font-serif font-bold leading-tight tracking-wide text-[hsl(var(--primary))] erdtree-text">{bug.title}</h1><div className="mt-3 flex items-center gap-3 font-mono text-[10px]  text-[hsl(var(--muted-foreground))]"><span>{bug.key}</span><span>/</span><span className="flex items-center gap-1.5"><span className="h-2 w-2 rounded-full shadow-[0_0_8px_currentColor]" style={{ background: component?.color, color: component?.color }} />{component?.name}</span><span>/</span><span>{relativeDate(bug.updatedAt)}</span></div><p className="mt-6 whitespace-pre-wrap border border-[hsl(var(--border)/.5)] bg-[hsl(var(--card)/.6)] backdrop-blur-md p-6 text-xl font-serif leading-relaxed text-[hsl(var(--foreground))]">{bug.description}</p></div></div></div>
        
        {/* Stage Progress Bar */}
        <div className="mb-10 border border-[hsl(var(--border)/.5)] bg-[hsl(var(--card)/.6)] backdrop-blur-md shadow-sm"><div className="flex flex-wrap items-center justify-between gap-3 border-b border-[hsl(var(--border)/.5)] bg-[hsl(var(--background)/.5)] px-6 py-4"><div className="flex items-center gap-3"><span className="font-serif text-[15px] font-bold  text-[hsl(var(--muted-foreground))]">Battle State</span><StageBadge stage={bug.stage} /></div><div className="flex items-center gap-2"><button onClick={() => move(-1)} disabled={bug.stage === stages[0]} className="border border-[hsl(var(--border))] bg-[hsl(var(--background))] p-2 text-[hsl(var(--muted-foreground))] hover:border-[hsl(var(--primary))] hover:text-[hsl(var(--primary))] disabled:opacity-30 transition-colors"><ArrowLeft size={14} /></button><button onClick={() => move(1)} disabled={bug.stage === stages[stages.length - 1]} className="border border-[hsl(var(--border))] bg-[hsl(var(--background))] p-2 text-[hsl(var(--muted-foreground))] hover:border-[hsl(var(--primary))] hover:text-[hsl(var(--primary))] disabled:opacity-30 transition-colors"><ArrowRight size={14} /></button></div></div><div className="flex items-center gap-2 overflow-x-auto px-6 py-5">{stages.map((stage, index) => <div key={stage} className="flex min-w-max items-center gap-2">{<button onClick={() => updateBug({ stage, resolution: stage === 'Closed' ? bug.resolution ?? 'Fixed' : null }, 'stage', bug.stage, stage)} className={`flex items-center gap-2 border px-4 py-2 text-base font-serif font-bold  transition-all ${stage === bug.stage ? 'border-[hsl(var(--primary))] bg-[hsl(var(--primary)/.1)] text-[hsl(var(--primary))] shadow-sm' : 'border-transparent text-[hsl(var(--muted-foreground))] hover:border-[hsl(var(--border))] hover:text-[hsl(var(--foreground))]'}`}><span className="h-2 w-2 rounded-full shadow-[0_0_8px_currentColor]" style={{ background: stageMeta[stage].tint, color: stageMeta[stage].tint }} />{stageMeta[stage].label}</button>}{index < stages.length - 1 && <ArrowRight size={14} className="text-[hsl(var(--border))] mx-1" />}</div>)}</div>{bug.stage === 'Closed' && <div className="flex flex-wrap items-center gap-4 border-t border-[hsl(var(--border)/.5)] bg-[hsl(var(--background)/.5)] px-6 py-4"><span className="font-serif text-[15px] font-bold  text-[hsl(var(--muted-foreground))]">Resolution</span><select value={bug.resolution ?? 'Fixed'} onChange={(event) => updateBug({ resolution: event.target.value as Resolution }, 'resolution', bug.resolution ?? '', event.target.value)} className="border border-[hsl(var(--border))] bg-[hsl(var(--card))] px-4 py-2 text-base font-serif font-bold  outline-none shadow-sm focus:border-[hsl(var(--primary))]">{resolutionOptions.map((value) => <option key={value}>{value}</option>)}</select></div>}</div>
        
        {/* Story vs Code Tabs */}
        <div className="mb-6 flex gap-1 border-b border-[hsl(var(--border)/.5)]">
          <button onClick={() => setActiveTab('story')} className={`flex items-center gap-2 px-6 py-3 text-lg font-serif font-bold  transition-all ${activeTab === 'story' ? 'border-b-2 border-[hsl(var(--primary))] text-[hsl(var(--primary))] bg-[hsl(var(--primary)/.05)]' : 'text-[hsl(var(--muted-foreground))] hover:text-[hsl(var(--foreground))] hover:bg-[hsl(var(--muted)/.5)]'}`}><MessageCircle size={16}/> Scribe's Tale <span className="font-sans normal-case tracking-normal opacity-70 text-[9px]">(Comments)</span></button>
          <button onClick={() => setActiveTab('diff')} className={`flex items-center gap-2 px-6 py-3 text-lg font-serif font-bold  transition-all ${activeTab === 'diff' ? 'border-b-2 border-[hsl(var(--primary))] text-[hsl(var(--primary))] bg-[hsl(var(--primary)/.05)]' : 'text-[hsl(var(--muted-foreground))] hover:text-[hsl(var(--foreground))] hover:bg-[hsl(var(--muted)/.5)]'}`}><Code2 size={16}/> Incantation <span className="font-sans normal-case tracking-normal opacity-70 text-[9px]">(Code Patch)</span></button>
        </div>

        {activeTab === 'story' ? (
          <section className="mb-7">
            <div className="space-y-4">{(!bug.comments || bug.comments.length === 0) ? <EmptyState title="No lore recorded" description="Be the first to inscribe the history of this foe." action={null} /> : bug.comments.map((item) => { const author = store.users.find((user) => user.id === item.authorId); return <div key={item.id} className="border border-[hsl(var(--border)/.5)] bg-[hsl(var(--card)/.6)] backdrop-blur-md p-6 shadow-sm"><div className="flex items-center gap-3"><Avatar user={author} /><span className="text-xl font-serif font-bold text-[hsl(var(--primary))]">{author?.name}</span><span className="border border-[hsl(var(--border))] bg-[hsl(var(--background)/.5)] px-2 py-1 font-serif text-[14px] font-bold  text-[hsl(var(--muted-foreground))]">{item.commentType}</span><span className="ml-auto font-mono text-[10px] text-[hsl(var(--muted-foreground))]">{relativeDate(item.createdAt)}</span></div><p className="mt-5 pl-[52px] text-xl font-serif leading-relaxed text-[hsl(var(--foreground))]">{item.body}</p></div>; })}</div>
            <div className="mt-6 border border-[hsl(var(--border)/.5)] bg-[hsl(var(--card)/.6)] backdrop-blur-md p-5 shadow-sm"><textarea value={comment} onChange={(event) => setComment(event.target.value)} placeholder="Inscribe your findings, a strategy, or a warning..." rows={4} className="focus-ring block w-full resize-none bg-transparent px-2 font-serif text-xl outline-none placeholder:text-[hsl(var(--muted-foreground))]" /><div className="mt-5 flex flex-wrap items-center justify-between gap-3 border-t border-[hsl(var(--border)/.5)] pt-5"><select value={commentType} onChange={(event) => setCommentType(event.target.value as CommentType)} className="border border-[hsl(var(--border))] bg-[hsl(var(--background))] px-4 py-2.5 text-base font-serif font-bold  outline-none shadow-sm">{['Info', 'Fix Proposed', 'Needs Repro', 'Verified'].map((value) => <option key={value}>{value}</option>)}</select><button onClick={addComment} disabled={!comment.trim()} className="flex items-center gap-2 bg-[hsl(var(--primary))] px-6 py-2.5 text-lg font-serif font-bold  text-black shadow-[0_0_15px_rgba(197,168,101,0.2)] disabled:opacity-50 hover:brightness-125 transition-all"><FileText size={14} /> Transcribe</button></div></div>
          </section>
        ) : (
          <section className="mb-7">
            {bug.codePatch ? (
      ...
      <aside className="space-y-6">
        {/* Metadata Sidebar */}
        <div className="border border-[hsl(var(--border)/.5)] bg-[hsl(var(--card)/.6)] backdrop-blur-md p-6 shadow-sm"><h2 className="mb-6 text-lg font-serif font-bold  text-[hsl(var(--primary))]">Attributes</h2><div className="space-y-5"><DetailRow label="Threat Level (Severity)"><SeverityBadge severity={bug.severity} /></DetailRow><DetailRow label="Urgency (Priority)"><PriorityPill priority={bug.priority} /></DetailRow><DetailRow label="Summoned (Assignee)"><div className="flex items-center gap-3"><Avatar user={assignee} /> <span className="text-xl font-serif font-bold">{assignee?.name ?? 'No Summon Sign'}</span></div></DetailRow><DetailRow label="Discoverer (Reporter)"><div className="flex items-center gap-3"><Avatar user={reporter} /> <span className="text-xl font-serif font-bold">{reporter?.name}</span></div></DetailRow><div className="my-4 h-px bg-[hsl(var(--border)/.5)]" /><DetailRow label="Runes Yielded (Points)"><span className="inline-flex items-center justify-center border border-[hsl(var(--primary)/.4)] bg-[hsl(var(--primary)/.1)] px-3 py-1.5 font-mono text-[11px] font-bold text-[hsl(var(--primary))] shadow-sm">{bug.storyPoints ? `${bug.storyPoints} Runes` : 'None'}</span></DetailRow><DetailRow label="Fated Day (Due Date)"><span className="text-lg font-serif font-bold">{bug.dueDate ? compactDate(bug.dueDate) : 'Undated'}</span></DetailRow><DetailRow label="Realm (Environment)"><span className="flex items-center gap-2 text-lg font-serif font-bold"><Castle size={14} className="text-[hsl(var(--primary))]"/>{bug.environment || 'Unknown'}</span></DetailRow><DetailRow label="Manifestation (Repro)"><span className="text-lg font-serif font-bold">{bug.reproducibility || 'Not Tried'}</span></DetailRow><DetailRow label="Sigils (Tags)"><div className="flex flex-wrap gap-2 mt-2">{(bug.tags||[]).length===0?<span className="text-base font-serif font-bold text-[hsl(var(--muted-foreground))]">None</span>:(bug.tags||[]).map(t=><span key={t} className="border border-[hsl(var(--border))] bg-[hsl(var(--background)/.5)] px-2.5 py-1 text-[9px] font-bold  text-[hsl(var(--primary))] shadow-sm">{t}</span>)}</div></DetailRow></div></div>
        
        {/* VISUAL DEPENDENCY TREE */}
        <div className="border border-[hsl(var(--border)/.5)] bg-[hsl(var(--card)/.6)] backdrop-blur-md p-6 shadow-sm">
          <div className="flex items-center justify-between mb-6"><h2 className="text-lg font-serif font-bold  text-[hsl(var(--primary))]">The Golden Lineage <span className="font-sans normal-case tracking-normal opacity-70 text-[9px]">(Graph)</span></h2><button onClick={() => setShowRelation(!showRelation)} className="p-1.5 text-[hsl(var(--muted-foreground))] hover:text-[hsl(var(--primary))] transition-colors"><Plus size={16} /></button></div>
          <div className="flex flex-col items-center gap-1.5">
            {dependencies.map(b => (
              <div key={`dep-${b.id}`} className="flex flex-col items-center w-full">
                <Link href={`/bugs/${b.id}`} className="w-full border border-[hsl(var(--destructive)/.5)] bg-[hsl(var(--destructive)/.1)] px-4 py-3 text-center text-base font-serif font-bold  text-[hsl(var(--destructive))] hover:bg-[hsl(var(--destructive)/.2)] transition-all shadow-sm"><ShieldAlert size={14} className="inline mr-2 mb-0.5"/> Blocks this: {b.key}</Link>
                <div className="h-6 w-px bg-gradient-to-b from-[hsl(var(--destructive))] to-[hsl(var(--primary))] my-1"></div>
              </div>
            ))}
            <div className="w-full border border-[hsl(var(--primary))] bg-[hsl(var(--primary)/.15)] px-4 py-3.5 text-center text-lg font-serif font-bold  text-[hsl(var(--primary))] shadow-[0_0_15px_rgba(197,168,101,0.2)]"><Skull size={16} className="inline mr-2 mb-0.5"/>{bug.key} (Current)</div>
            {blockers.map(b => (
              <div key={`blk-${b.id}`} className="flex flex-col items-center w-full">
                ...
                <Link href={`/bugs/${b.id}`} className="w-full border border-[hsl(var(--accent)/.5)] bg-[hsl(var(--accent)/.1)] px-4 py-3 text-center text-base font-serif font-bold  text-[hsl(var(--accent))] hover:bg-[hsl(var(--accent)/.2)] transition-all shadow-sm"><AlertCircle size={14} className="inline mr-2 mb-0.5"/> Blocked by this: {b.key}</Link>
              </div>
            ))}
            {dependencies.length === 0 && blockers.length === 0 && !showRelation && <p className="text-base font-serif font-bold text-[hsl(var(--muted-foreground))] text-center mt-3">No lineage linked.</p>}
          </div>
          {showRelation && <RelationshipForm bug={bug} onDone={() => setShowRelation(false)} />}
        </div>
        
        {/* ATTACHMENT VIEWER */}
        <div className="border border-[hsl(var(--border)/.5)] bg-[hsl(var(--card)/.6)] backdrop-blur-md p-6 shadow-sm"><div className="flex items-center justify-between mb-5"><h2 className="flex items-center gap-2 text-lg font-serif font-bold  text-[hsl(var(--primary))]"><Eye size={16} /> Visions <span className="font-sans normal-case tracking-normal opacity-70 text-[9px]">(Media)</span></h2><label className="cursor-pointer p-1.5 text-[hsl(var(--muted-foreground))] hover:text-[hsl(var(--primary))] transition-colors"><Plus size={16} /><input type="file" onChange={addAttachment} className="hidden" /></label></div>{(!bug.attachments || bug.attachments.length === 0) ? <p className="mt-2 text-base font-serif leading-5 text-[hsl(var(--muted-foreground))] font-bold">No visions recorded.</p> : <div className="mt-4 grid grid-cols-2 gap-3">{bug.attachments.map((item) => <a key={item.id} href={item.fileUrl} target="_blank" rel="noreferrer" className="group relative block aspect-square overflow-hidden border border-[hsl(var(--border))] bg-[hsl(var(--muted))] shadow-sm transition-transform hover:border-[hsl(var(--primary))]">{item.fileUrl.startsWith('data:image') ? <img src={item.fileUrl} alt="attachment" className="h-full w-full object-cover opacity-80 group-hover:opacity-100 transition-opacity" /> : <div className="flex h-full items-center justify-center"><FileText size={28} className="text-[hsl(var(--primary))]" /></div>}<div className="absolute inset-x-0 bottom-0 bg-black/80 pt-6 pb-3 px-3 opacity-0 transition-opacity group-hover:opacity-100"><p className="truncate text-[15px] font-serif font-bold  text-[hsl(var(--primary))]">{item.fileName}</p></div></a>)}</div>}</div>
        
        <div className="border border-[hsl(var(--border)/.5)] bg-[hsl(var(--card)/.6)] backdrop-blur-md p-6 shadow-sm"><h2 className="mb-6 flex items-center gap-2 text-lg font-serif font-bold  text-[hsl(var(--primary))]"><Clock3 size={16} /> Annals of Time <span className="font-sans normal-case tracking-normal opacity-70 text-[9px]">(History)</span></h2>{(!bug.history || bug.history.length === 0) ? <p className="text-base font-serif font-bold text-[hsl(var(--muted-foreground))]">No history recorded.</p> : <div className="space-y-5">{bug.history.slice(0, 5).map((entry) => <div key={entry.id} className="relative pl-6 text-lg font-serif before:absolute before:left-0 before:top-1.5 before:h-2 before:w-2 before:rotate-45 before:bg-[hsl(var(--primary))] before:shadow-[0_0_8px_hsl(var(--primary))]"><p className="font-bold text-[hsl(var(--foreground))]">{entry.fieldChanged} <span className="font-normal text-[hsl(var(--muted-foreground))]">altered</span></p><p className="mt-1 font-mono text-[10px] text-[hsl(var(--muted-foreground))]">{entry.oldValue || 'void'} → {entry.newValue}</p></div>)}</div>}</div>
      </aside>
    </div>
    {toast && <div className="fixed bottom-8 left-1/2 z-50 flex -translate-x-1/2 items-center gap-3 border border-[hsl(var(--primary))] bg-black px-6 py-3.5 text-lg font-serif font-bold  text-[hsl(var(--primary))] shadow-[0_0_30px_rgba(197,168,101,0.3)]"><Check size={16} className="text-[hsl(var(--primary))]" />{toast}</div>}
  </div>;
}

function DetailRow({ label, children }: { label: string; children: ReactNode }) { return <div><p className="mb-2 font-serif text-[15px] font-bold  text-[hsl(var(--muted-foreground))]">{label}</p>{children}</div>; }
...
function EditPanel({ bug, onSave }: { bug: Bug; onSave: (patch: Partial<Bug>) => void }) {
  ...
  return <div className="mb-8 border border-[hsl(var(--primary)/.3)] bg-[hsl(var(--primary)/.05)] p-6 shadow-sm"><div className="mb-6 flex items-center gap-3"><Settings2 size={18} className="text-[hsl(var(--primary))]" /><h2 className="text-xl font-serif font-bold  text-[hsl(var(--primary))]">Amend Records <span className="font-sans normal-case tracking-normal opacity-70 text-[9px]">(Edit)</span></h2></div><div className="space-y-5"><input value={title} onChange={(e) => setTitle(e.target.value)} className="w-full border border-[hsl(var(--border))] bg-[hsl(var(--background))] px-4 py-3 text-xl font-serif font-bold outline-none focus:border-[hsl(var(--primary))] text-[hsl(var(--foreground))]" /><textarea value={description} onChange={(e) => setDescription(e.target.value)} rows={3} className="w-full resize-none border border-[hsl(var(--border))] bg-[hsl(var(--background))] px-4 py-3 text-xl font-serif outline-none focus:border-[hsl(var(--primary))] text-[hsl(var(--foreground))]" /><div className="grid grid-cols-3 gap-4"><select value={severity} onChange={(e) => setSeverity(e.target.value as Severity)} className="border border-[hsl(var(--border))] bg-[hsl(var(--background))] px-3 py-3 text-lg font-serif font-bold outline-none">{Object.keys(severityMeta).map((v) => <option key={v}>{v}</option>)}</select><select value={priority} onChange={(e) => setPriority(e.target.value as Priority)} className="border border-[hsl(var(--border))] bg-[hsl(var(--background))] px-3 py-3 text-lg font-serif font-bold outline-none">{['P0', 'P1', 'P2', 'P3'].map((v) => <option key={v}>{v}</option>)}</select><select value={assigneeId} onChange={(e) => setAssigneeId(e.target.value)} className="border border-[hsl(var(--border))] bg-[hsl(var(--background))] px-3 py-3 text-lg font-serif font-bold outline-none"><option value="">No summon</option>{(store.users || []).map((u) => <option key={u.id} value={u.id}>{u.name}</option>)}</select></div></div><div className="mt-6 flex justify-end"><button onClick={submitAction} className="bg-[hsl(var(--primary))] px-6 py-2.5 text-lg font-serif font-bold  text-black shadow-md hover:brightness-125 transition-all">Seal Edits <span className="font-sans normal-case tracking-normal opacity-70">(Save)</span></button></div></div>;
}
...
function RelationshipForm({ bug, onDone }: { bug: Bug; onDone: () => void }) {
  ...
  return <div className="mt-5 border border-[hsl(var(--border))] bg-[hsl(var(--background))] p-5"><select value={kind} onChange={(e) => setKind(e.target.value as 'dependsOn' | 'duplicateOf')} className="mb-3 block w-full border border-[hsl(var(--border))] bg-[hsl(var(--card))] px-3 py-2.5 text-base font-serif font-bold  outline-none"><option value="dependsOn">Blocks this</option><option value="duplicateOf">Duplicate of</option></select><select value={target} onChange={(e) => setTarget(e.target.value)} className="block w-full border border-[hsl(var(--border))] bg-[hsl(var(--card))] px-3 py-2.5 text-base font-serif font-bold  outline-none">{candidates.map((item) => <option key={item.id} value={item.id}>{item.key} · {item.title}</option>)}</select><div className="mt-4 flex justify-end gap-3"><button onClick={onDone} className="px-4 py-2 text-[15px] font-serif font-bold  text-[hsl(var(--muted-foreground))] hover:text-[hsl(var(--foreground))]">Flee</button><button onClick={submitAction} className="bg-[hsl(var(--primary))] px-5 py-2 text-[15px] font-serif font-bold  text-black shadow-sm hover:brightness-125">Forge Link</button></div></div>;
}
...
function DashboardPage() {
  ...
  return <div className="mx-auto w-full max-w-[1600px] px-5 py-8 md:px-10 lg:px-12 relative z-10"><SectionHeader eyebrow={`Records of ${currentWs?.name ?? ''}`} title="Runes & Records" description="Behold the state of the Golden Order. Seek the imbalances and cleanse them." action={<button onClick={() => window.location.reload()} className="flex items-center gap-2 border border-[hsl(var(--border)/.5)] bg-[hsl(var(--card)/.6)] backdrop-blur-md px-5 py-2.5 text-lg font-serif font-bold  shadow-sm hover:border-[hsl(var(--primary))] hover:text-[hsl(var(--primary))] transition-colors"><RefreshCw size={14}/> Divining <span className="font-sans text-[9px] tracking-normal normal-case opacity-70">(Refresh)</span></button>} />
    <div className="grid gap-6 md:grid-cols-[1.25fr_.75fr]"><div className="border border-[hsl(var(--border)/.5)] bg-[hsl(var(--card)/.6)] backdrop-blur-md p-8 shadow-sm"><h2 className="mb-8 text-2xl font-serif font-bold tracking-wide  text-[hsl(var(--primary))] erdtree-text">State of the Realm</h2><div className="space-y-6">{stages.map((stage) => { const count = workspaceBugs.filter((bug) => bug.stage === stage).length; const meta = stageMeta[stage]; return <div key={stage}><div className="mb-2.5 flex justify-between text-[12px]"><span className="flex items-center gap-3 font-serif font-bold  tracking-wider text-[hsl(var(--foreground))]"><span className="h-3 w-3 rounded-full shadow-[0_0_10px_currentColor]" style={{ background: meta.tint, color: meta.tint }} />{meta.label}</span><span className="font-serif font-bold text-[hsl(var(--muted-foreground))]">{count} Foes</span></div><div className="h-3 overflow-hidden bg-[hsl(var(--background)/.5)] border border-[hsl(var(--border)/.5)]"><div className="h-full transition-all duration-1000 ease-out" style={{ width: `${(count / maxStage) * 100}%`, background: meta.tint }} /></div></div>; })}</div></div><div className="border border-[hsl(var(--primary)/.5)] bg-black/80 backdrop-blur-xl p-10 shadow-[0_0_40px_rgba(197,168,101,0.15)] relative overflow-hidden"><div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[300px] h-[300px] bg-[hsl(var(--primary)/.1)] rounded-full blur-[60px] pointer-events-none"></div><p className="relative z-10 font-serif text-base font-bold  tracking-[0.3em] text-[hsl(var(--primary))]">Impending Doom</p><p className="relative z-10 mt-6 text-8xl font-serif font-bold tracking-tighter text-[hsl(var(--foreground))] drop-shadow-md">{open.filter((bug) => bug.priority === 'P0' || bug.stage === 'Blocked').length}</p><p className="relative z-10 mt-4 text-xl font-serif font-bold   text-[hsl(var(--destructive))]">Critical Threats</p><p className="relative z-10 mt-10 border-t border-[hsl(var(--border))] pt-6 text-[19px] font-serif leading-relaxed text-[hsl(var(--muted-foreground))]">These foes pose an immediate threat to the Erdtree. They carry the mark of Destined Death or block the path of other champions.</p></div></div>
    <div className="mt-6 grid gap-6 sm:grid-cols-3">
      <div className="border border-[hsl(var(--border)/.5)] bg-[hsl(var(--card)/.6)] backdrop-blur-md p-8 shadow-sm"><div className="flex items-center justify-between"><p className="text-base font-serif font-bold  text-[hsl(var(--muted-foreground))]">Age of Conflict</p><Clock3 size={20} className="text-[hsl(var(--primary))]" /></div><p className="mt-5 font-serif text-5xl font-bold tracking-tight text-[hsl(var(--foreground))]">{avgAge} <span className="text-xl text-[hsl(var(--muted-foreground))]">Days</span></p><p className="mt-3 text-base font-serif italic text-[hsl(var(--muted-foreground))]">average time foes roam unchecked</p></div>
      <div className="border border-[hsl(var(--border)/.5)] bg-[hsl(var(--card)/.6)] backdrop-blur-md p-8 shadow-sm"><div className="flex items-center justify-between"><p className="text-base font-serif font-bold  text-[hsl(var(--muted-foreground))]">Victories (Monthly)</p><Crown size={20} className="text-[hsl(var(--primary))]" /></div><p className="mt-5 font-serif text-5xl font-bold tracking-tight text-[hsl(var(--foreground))]">{workspaceBugs.filter((bug) => bug.stage === 'Closed').length}</p><p className="mt-3 text-base font-serif italic text-[hsl(var(--muted-foreground))]">foes felled and recorded</p></div>
      <div className="border border-[hsl(var(--destructive)/.3)] bg-[hsl(var(--card)/.6)] backdrop-blur-md p-8 shadow-sm"><div className="flex items-center justify-between"><p className="text-base font-serif font-bold  text-[hsl(var(--destructive))]">Demigods & Great Foes</p><Skull size={20} className="text-[hsl(var(--destructive))]" /></div><p className="mt-5 font-serif text-5xl font-bold tracking-tight text-[hsl(var(--foreground))]">{workspaceBugs.filter((bug) => bug.severity === 'Critical' || bug.severity === 'Major').length}</p><p className="mt-3 text-base font-serif italic text-[hsl(var(--muted-foreground))]">high threat targets active</p></div>
    </div>
  </div>;
}
      ...
      {stages.map((stage) => {
        ...
        return (
          <div key={stage} className="flex w-[320px] shrink-0 snap-center flex-col border border-[hsl(var(--border)/.5)] bg-[hsl(var(--background)/.6)] backdrop-blur-md p-3 shadow-sm" onDragOver={(e) => e.preventDefault()} onDrop={(e) => { const bugId = e.dataTransfer.getData('text/plain'); if (bugId) onMove(bugId, stage); }}>
            <div className="mb-4 flex items-center justify-between px-2 pt-2 border-b border-[hsl(var(--border)/.5)] pb-3"><div className="flex items-center gap-3"><span className="h-2 w-2 rounded-full shadow-[0_0_8px_currentColor]" style={{ background: meta.tint, color: meta.tint }} /><span className="font-serif text-[19px] font-bold  text-[hsl(var(--foreground))]">{meta.label}</span></div><span className="font-serif text-xl font-bold text-[hsl(var(--muted-foreground))]">{stageBugs.length}</span></div>
              ...
              {stageBugs.map((bug) => {
                ...
                return (
                  <div key={bug.id} draggable onDragStart={(e) => e.dataTransfer.setData('text/plain', bug.id)} onClick={() => setLocation(`/bugs/${bug.id}`)} className="group cursor-grab border border-[hsl(var(--border))] bg-[hsl(var(--card)/.9)] backdrop-blur-md p-5 shadow-sm transition-all hover:-translate-y-1 hover:border-[hsl(var(--primary))] hover:shadow-[0_5px_20px_rgba(197,168,101,0.1)] active:cursor-grabbing relative overflow-hidden">
                    ...
                    <div className="mb-3 flex justify-between gap-3 items-start"><span className="font-mono text-[10px] font-bold  text-[hsl(var(--muted-foreground))] group-hover:text-[hsl(var(--primary))] transition-colors">{bug.key}</span><Avatar user={assignee} /></div>
                    <p className="font-serif text-[21px] font-bold leading-snug tracking-wide text-[hsl(var(--foreground))] mb-4">{bug.title}</p>
                    <div className="flex flex-wrap gap-2 overflow-hidden mb-4">{(bug.tags || []).slice(0,3).map(t => <span key={t} className="truncate border border-[hsl(var(--border))] bg-[hsl(var(--background)/.5)] px-2 py-0.5 text-[9px] font-bold  text-[hsl(var(--muted-foreground))]">{t}</span>)}</div>
